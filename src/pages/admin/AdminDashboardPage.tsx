...
interface RecentOrder {
    ...
    status: string; createdAt: string; itemCount?: number;
}
...
const AdminDashboardPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchStats = async () => {
             ...
             try {
                ...
                const productResponse = await adminApi.get('/api/products');
                // *** End Path Correction ***
                setStats({ productCount: Array.isArray(productResponse.data) ? productResponse.data.length : 0 });
            } catch (err: any) {
                 console.error("Stats fetch error:", err);
                 if(!err.handled){ setStatsError(err.response?.data?.message || 'Failed to load dashboard stats.'); }
                 setStats(null);
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            ...
            try {
                ...
                const response = await adminApi.get<RecentOrder[]>('/api/admin/orders', {
                    params: { limit: 5 }
                });
                 // *** End Path Correction ***
                setRecentOrders(Array.isArray(response.data) ? response.data : []);
                 ...
                 if (!err.handled) { setOrdersError(err.response?.data?.message ||'Failed to load recent orders.'); }
                 setRecentOrders([]);
    ...
    return (
        <div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md"><h2 className="text-lg font-semibold text-gray-700 mb-4 flex items-center"><ListOrdered className="h-5 w-5 mr-2 text-gray-500"/> Recent Orders</h2>{isLoadingOrders && <div className="flex justify-center py-4"><BarLoader color="#14B8A6" /></div>}{ordersError && !isLoadingOrders && ( <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded flex items-center" role="alert"><AlertCircle className="h-5 w-5 mr-2 flex-shrink-0"/> Error: {ordersError}</div> )}{!isLoadingOrders && !ordersError && recentOrders.length === 0 && ( <p className="text-gray-500 text-sm text-center py-4">No recent orders found.</p> )}{!isLoadingOrders && !ordersError && recentOrders.length > 0 && ( <div className="overflow-x-auto"><table className="min-w-full divide-y divide-gray-200"><thead className="bg-gray-50"><tr><th scope="col" className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Date</th><th scope="col" className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ref</th><th scope="col" className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Customer</th><th scope="col" className="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Amount</th><th scope="col" className="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th></tr></thead><tbody className="bg-white divide-y divide-gray-200">{recentOrders.map((order) => ( <tr key={order.id} className="hover:bg-gray-50 transition-colors"><td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{formatDate(order.createdAt)}</td><td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-800" title={order.merchantReference}>{order.merchantReference.length > 15 ? order.merchantReference.substring(0, 15) + '...' : order.merchantReference}</td><td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{order.customerName}</td><td className="px-4 py-3 whitespace-nowrap text-sm text-gray-800 font-semibold text-right">{formatPrice(order.totalAmount)}</td><td className="px-4 py-3 whitespace-nowrap text-sm text-center"><span className={`px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${ order.status === 'PAID' ? 'bg-green-100 text-green-800' : order.status === 'PENDING' ? 'bg-yellow-100 text-yellow-800' : order.status === 'SHIPPED' ? 'bg-blue-100 text-blue-800' : order.status === 'DELIVERED' ? 'bg-indigo-100 text-indigo-800' : order.status === 'FAILED' || order.status.includes('FAIL') || order.status.includes('ERROR') ? 'bg-red-100 text-red-800' : order.status === 'CANCELLED' ? 'bg-gray-100 text-gray-700' : 'bg-gray-100 text-gray-800' }`}>{order.status}</span></td></tr> ))}</tbody></table></div> )}</div>
